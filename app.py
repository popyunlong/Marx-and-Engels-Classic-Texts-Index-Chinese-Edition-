...
# **USM=0（不锐化）**：实测列宁 60 卷中约 52 卷源仅 ~700-800px，按原生放大到显示下限本就是
# 较大插值；再叠 USM 会把扫描底噪/JPEG 颗粒一并放大成毛刺/黑边，肉眼反而"更糊更脏"（用户实测
# 列宁部分卷不如旧版清晰即此故）。清晰度提升**只靠更高渲染分辨率 + PyMuPDF 的好插值**即可，
# 锐化对这类灰底扫描得不偿失。高清卷（vol03 等 native≥1600）本就按原生渲染、无需锐化。
DEFAULT_RENDER = {

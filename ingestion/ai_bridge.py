...
def active_upstream(config_path=Path('/etc/caddy/Caddyfile')):
    text=config_path.read_text(encoding='utf-8')
    # Follow the actual default application upstream during any code or corpus
    # rollout. Named matchers (including this bridge) must never be candidates.
    ports=re.findall(r'(?m)^\s*reverse_proxy\s+127\.0\.0\.1:([0-9]{1,5})(?:\s*\{)?\s*$',text)
    if len(set(ports))!=1 or not 1 <= int(ports[0]) <= 65535 or int(ports[0]) == LISTEN[1]:
        raise RuntimeError('当前网站入口无法唯一确认')
    return f'http://127.0.0.1:{ports[0]}/api/ai/search-chat'
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            req = urllib.request.Request(active_upstream(), data=body, headers=headers, method="POST")
            try:

...
def main():
    ...
    parser.add_argument('--foundation-release', default='')
    ...
    for row in coverage:
        row['发布状态'] = '基础代码已上线；目录数据修复未上线' if args.foundation_release else '未上线'
        ...
        if 'mega2-iv-3/index.html' in row['来源']:
            row['本轮候选修复'] = '后续批候选：按 1998 年印刷目录建立 50 项五级正文目录，补 12 个精确锚点；保留旧分页入口'
            row['本轮浏览器验证'] = '隔离候选网页在 1280、390、360 像素验证；生产候选实例待验收'
            row['新增证据缺口'] = '现有原书 PDF 只有 Text；Apparat 和 Register 缺失，正文独立标题覆盖待核对'
    for issue in issues:
        issue['本轮状态'] = ('基础代码已上线；目录数据未修，保留原审计结论' if args.foundation_release
                         else '未上线；保留原审计结论，待逐批修复验收')
    ...
    status_line = ('基础兼容代码已上线；目录数据修复尚未上线。' if args.foundation_release
                   else '基础兼容代码及目录数据修复尚未上线。')
    overview_line = ('基础发布仍须通过观察门槛。' if args.foundation_release
                     else '基础发布仍须完成生产候选验收。')
...
<h1>目录修复候选交付</h1><p class="status">__STATUS__覆盖表保留全部 777 个审计单元的原核查结论，候选修复不等于整卷核实通过。</p>
<p>已准备完整基线和首批定向修复。第二批包含《文集》5 卷及 MEGA² II/5 的有界试点；MEGA² IV/3 候选正在逐项核对。当前任务已被指定为发布协调者；__OVERVIEW__逐卷验收仍是后续批次的放行条件。</p>
<input id="filter" placeholder="筛选书名、卷册、来源或候选修复"><div class="table"><table><thead><tr>'''
    ...
    page = page.replace('__STATUS__', html.escape(status_line)).replace('__OVERVIEW__', html.escape(overview_line))
    (output / '逐卷修复状态.html').write_text(page, encoding='utf-8')
    print('Coverage:', len(coverage), 'Issue records:', len(issues),
          'Published foundation:', bool(args.foundation_release))

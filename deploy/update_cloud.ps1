...
try {
    ...
    Invoke-Remote-BestEffort "cd '$RemoteDir' && chown www-data:www-data *.py templates/*.html scripts/*.py deploy/*.ps1 deploy/*.sh deploy/marx-search-journal-alerts.* deploy/marx-search-journal-send.* config/*.example config/books.yaml config/manifest.yaml config/volumes.yaml config/wenji_toc_overrides.yaml config/quanji_toc_overrides.yaml data/dictionary.sqlite data/dictionary_polish_report.json DEPLOY_SERVER.md 2>/dev/null || true && chmod -R a+rX templates scripts deploy config static 2>/dev/null || true && chmod a+r static/vendor/qrcode.min.js data/dictionary.sqlite data/dictionary_polish_report.json 2>/dev/null || true"
